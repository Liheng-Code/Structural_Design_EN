type CheckResult=any; type Project=any; type Status=any;
...
export function beamFem(opts: {
  ...
  // summing the net transverse load (applied pressure plus Winkler foundation reaction,
  // plus discrete spring reactions) along the beam: V(x)=∫w dx, M(x)=∫V dx. Both ends of
  // the pile/sheet-pile are physically free (no other supports besides the modelled
  // springs), so this is exact and self-consistent (dM/dx≡V by construction), unlike a
  // per-element Hermite-curvature recovery which does not use a consistent shear formula.
