...
import type { WindLoadCheckStatus, WindLoadProject, WindLoadAnalysisResult } from "@/lib/wind-load/types";
import { analyzeWindLoad } from "@/lib/wind-load/calculations";

export type Tab = "overview" | "site" | "geometry" | "pressure" | "force-coeff" | "wind-force" | "dynamic" | "report";

export type Pad = (patch: Partial<WindLoadProject>) => void;

export interface Body {
  project: WindLoadProject;
  res: ReturnType<typeof analyzeWindLoad>;
  pad: Pad;
}

export type Results = WindLoadAnalysisResult;

export function StatusBadge({ status }: { status: WindLoadCheckStatus }) {
  const cls =
    status === "PASS"
