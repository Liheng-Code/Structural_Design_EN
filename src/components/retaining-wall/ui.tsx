...
import type { CantileverWallCheckStatus, CantileverWallProject } from "@/lib/retaining-wall/types";
import type { CantileverWallAnalysisResult } from "@/lib/retaining-wall/types";
import { analyzeCantileverWall } from "@/lib/retaining-wall/calculations";

export type Tab = "overview" | "geometry" | "soil" | "stability" | "stem" | "base" | "detailing" | "report";

export type Pad = (patch: Partial<CantileverWallProject>) => void;

export interface Body {
  project: CantileverWallProject;
  res: ReturnType<typeof analyzeCantileverWall>;
  pad: Pad;
}

export type Results = CantileverWallAnalysisResult;

export function StatusBadge({ status }: { status: CantileverWallCheckStatus }) {
  const cls =
    status === "PASS"
