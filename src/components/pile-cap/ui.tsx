...
import type { PileCapCheckStatus, PileCapProject } from "@/lib/pile-cap/types";
import type { PileCapAnalysisResult } from "@/lib/pile-cap/types";
import { analyzePileCap } from "@/lib/pile-cap/calculations";

export type Tab = "overview" | "geometry" | "stm" | "shear" | "sls" | "detailing" | "report";

export type Pad = (patch: Partial<PileCapProject>) => void;

export interface Body {
  project: PileCapProject;
  res: ReturnType<typeof analyzePileCap>;
  pad: Pad;
}

export type Results = PileCapAnalysisResult;

export function StatusBadge({ status }: { status: PileCapCheckStatus }) {
  const cls =
    status === "PASS"
            ...
            <tr key={c.id} className="hover:bg-slate-900/50">
              ...
              <td
                className={`p-3 text-right font-bold ${
                  c.utilization > 1 ? "text-rose-400" : c.utilization > 0.9 ? "text-amber-400" : "text-emerald-400"
                }`}
              >
...
export function Eq({ children }: { children: React.ReactNode }) {
  return (
    <div className="rounded-lg bg-[#03070e] border border-cyan-900/60 p-3 font-mono text-xs text-cyan-100 overflow-x-auto">
      {children}
    </div>
  );
}